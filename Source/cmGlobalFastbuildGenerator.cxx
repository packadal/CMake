...
complaints and fails generation?
   ...
   - Linker for msvc uses the cmake command to callback. Don't think this is an
issue since
...
public:
  ...
  static bool WriteCompilers(GenerationContext& context)
  {
    ...
    for (StringMap::iterator iter = compilerToCompilerName.begin();
         iter != compilerToCompilerName.end(); ++iter) {
      ...
      std::string compilerFile =
        "$CompilerRoot$\\" + cmSystemTools::GetFilenameName(compilerLocation);
    ...
    for (StringMap::iterator iter = languageToCompiler.begin();
      ...
      const std::string& compilerName =
        compilerToCompilerName[compilerLocation];
                                        ...
                                        cmCustomCommandGenerator const& ccg)
  {
    ...
    if (!outputs.empty()) {
      cmLocalGenerator::RelativeRoot relative_root =
        ccg.GetWorkingDirectory().empty() ? cmLocalGenerator::START_OUTPUT
                                 ...
                                 const std::string& targetName)
  {
    ...
    // outputs.
    {
      ...
      for (size_t index = 0; index < mergedOutputs.size(); ++index) {
        ...
        std::vector<cmSourceFile*>::iterator findResult =
          std::find_if(sourceFiles.begin(), sourceFiles.end(),
                       SourceFileNameIs(outputName));
    ...
    for (; iter != end; ++iter) {
      ...
      if (!customCommands.empty()) {
        ...
        std::string customCommandNameBase =
          targetName + "-CustomCommand-" + configName + "-";
    ...
    for (std::set<std::string>::iterator langIter = languages.begin();
      ...
      for (; configIter != configEnd; ++configIter) {
        ...
        context.fc.WriteComment("Compiler options:");
        {
          ...
          context.fc.WriteVariable("CompilerOutputPath",
                                   Quote(targetCompileOutDirectory));

          std::string compileObjectCmd =
            Detection::DetectCompileRule(lg, target, objectGroupLanguage);
          // context.fc.WriteVariable("CompilerRuleCmd", Quote(
          // compileObjectCmd ));
        }
