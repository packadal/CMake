...
#define FASTBUILD_DOLLAR_TAG "FASTBUILD_DOLLAR_TAG"
        ...
        const std::string & configName)
    {
        ...
        if (generatorTarget->HaveWellDefinedOutputFiles())
        {
            ...
            targetNamesOut.targetOutputReal = generatorTarget->GetFullPath(configName,
                /*implib=*/false,
                /*realpath=*/true);
            targetNamesOut.targetOutputImplib = generatorTarget->GetFullPath(configName,
                /*implib=*/true);
        }
        else
        {
            targetNamesOut.targetOutputDir = lg->GetMakefile()->GetHomeOutputDirectory();
        ...
        {
            targetNamesOut.targetOutputPDBDir = generatorTarget->GetPDBDirectory(configName);
            targetNamesOut.targetOutputPDBDir += "/";
        }
        if (generatorTarget->GetType() <= cmState::OBJECT_LIBRARY)
        {
            targetNamesOut.targetOutputCompilePDBDir = generatorTarget->GetCompilePDBPath(configName);
            if (targetNamesOut.targetOutputCompilePDBDir.empty())
            {
                targetNamesOut.targetOutputCompilePDBDir = generatorTarget->GetSupportDirectory() + "/";
            }
        }
        ...
        const std::string& language)
    {
        ...
        lg->AddCompileDefinitions(defines, generatorTarget,
            configName, language);
        ...
        const std::string & configName )
    {
        ...
        cmComputeLinkInformation* pcli = generatorTarget->GetLinkInformation(configName);
        if (!pcli)
        {
        ...
        const std::string& configName)
    {
        ...
        lg->AddVisibilityPresetFlags(compileFlags, gt,
            language);
        ...
        //FIXME const cast are evil
        std::string includeFlags = lg->GetIncludeFlags(includes, (cmGeneratorTarget*)gt,
            language,
...
public:
        ...
        TargetContextMap& map)
    {
        ...
        for(; it != end; ++it)
        {
                ...
                iter != generators.end(); ++iter)
            {
                ...
                for (std::vector<cmGeneratorTarget*>::const_iterator targetIter = lg->GetGeneratorTargets().begin();
                    targetIter != lg->GetGeneratorTargets().end();
                    ++targetIter)
                {
                    ...
                    TargetGenerationContext targetContext =
                        { generatorTarget, root, generators, lg };
        ...
        const std::vector<std::string>& orderDeps)
    {
        ...
        for (; iter != end; ++iter)
        {
                ...
                ccIter != commands.end(); ++ccIter)
            {
                ...
                WriteCustomCommand(context, &cc, lg, configName, customCommandTargetNameStr,
                    targetName);
            ...
            if (!customCommands.empty())
            {
                    ...
                    ccIter != customCommands.end(); ++ccIter)
                {
                    ...
                    WriteCustomCommand(context, sourceFile->GetCustomCommand(),
                        lg, configName, customCommandTargetNameStr,
                        targetName);
    ...
    static void WriteTargetDefinition(GenerationContext& context,
        cmLocalFastbuildGenerator *lg, const cmGeneratorTarget *generatorTarget)
    {
        ...
        std::vector<std::string> dependencies;
        Detection::DetectTargetCompileDependencies(context.self, generatorTarget, dependencies);
        ...
        for (; configIter != configEnd; ++configIter)
        {
            ...
            // Write out the output paths for the outcome of this target
            {
                ...
                Detection::DetectOutput(lg, targetNames, generatorTarget, configName);
        ...
        WriteCustomBuildSteps(context, lg, generatorTarget, generatorTarget->GetPreBuildCommands(), "PreBuild", dependencies);
        WriteCustomBuildSteps(context, lg, generatorTarget, generatorTarget->GetPreLinkCommands(), "PreLink", dependencies);
        ...
        bool hasCustomBuildRules = WriteCustomBuildRules(context, lg, generatorTarget);

        // Figure out the list of languages in use by this target
        std::vector<std::string> linkableDeps;
        std::vector<std::string> orderDeps;
        std::set<std::string> languages;
        Detection::DetectLanguages(languages, context.self, generatorTarget);
            ...
            for (; configIter != configEnd; ++configIter)
            {
                ...
                std::string baseCompileFlags;
                {
                    ...
                    Detection::DetectBaseCompileCommand(compileCmd,
                        lg, generatorTarget, objectGroupLanguage);
                ...
                context.fc.WriteComment("Source files:");
                {
                        ...
                        sourceIter != filteredObjectSources.end(); ++sourceIter)
                    {
                        ...
                        Detection::DetectCompilerFlags(compilerFlags,
                            lg, generatorTarget, srcFile, objectGroupLanguage, configName);
                        std::string compileDefines =
                            Detection::ComputeDefines(lg, generatorTarget, srcFile, configName, objectGroupLanguage);
                    ...
                    ++groupIter)
                {
                    ...
                    for(objectListIt = command.sourceFiles.begin(); objectListIt != command.sourceFiles.end(); ++objectListIt)
                    {
                        ...
                        std::string targetCompileOutDirectory =
                            Detection::DetectTargetCompileOutputDir(lg, generatorTarget, configName);
        ...
        for (; iter != end; ++iter)
        {
            ...
            if (hasLinkerStage)
            {
                ...
                // Linker options
                {
                    ...
                    std::string linkPath;
                    Detection::DetectLinkerLibPaths(linkPath, lg, generatorTarget, configName);
                    ...
                    if(generatorTarget->IsExecutableWithExports())
                    {
                        ...
                        const cmSourceFile* defFile = generatorTarget->GetModuleDefinitionFile(configName);
                    ...
                    // Now detect the extra dependencies for linking
                    {
                        ...
                        Detection::DetectTargetObjectDependencies( context.self, generatorTarget, configName, extraDependencies );
                        Detection::DetectTargetLinkDependencies(generatorTarget, configName, extraDependencies);
        ...
        WriteCustomBuildSteps(context, lg, generatorTarget, generatorTarget->GetPostBuildCommands(), "PostBuild",
        ...
        WriteTargetAliases(context, generatorTarget, linkableDeps, orderDeps);
...
//----------------------------------------------------------------------------
cmLocalGenerator *cmGlobalFastbuildGenerator::CreateLocalGenerator(cmMakefile* makefile)
{
